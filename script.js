...
const avatarPosition = {
  1:{bottom:'10%',left:'50%'},2:{bottom:'0.5%',left:'50%'},3:{bottom:'6%',left:'50%'},
  4:{bottom:'6%',left:'50%'},5:{bottom:'7%',left:'50%'},6:{bottom:'9%',left:'50%'},7:{bottom:'4%',left:'45%'}
};
const avatarSize = {1:'50%',2:'80%',3:'50%',4:'50%',5:'50%',6:'50%',7:'50%'};
...
//-------------------------------------------Event Listeners----------------------------------------------------
// submit, restart, print, hint buttons must exist in your HTML
...
function startGame(gender){
  ...
  if(!playerName){nameInput.classList.add('input-error','shake');sounds.nameError.play();setTimeout(()=>nameInput.classList.remove('shake'),500);return;}
  ...
  hintUsed=false; setupLevel(currentLevel); startTimer();
  sounds.newGame.play(); sounds.ticking.loop=true; sounds.ticking.play();
}

function setupLevel(levelNum){
  hintUsed=false; const btn=document.getElementById('hint-button'); if(btn)btn.disabled=false;
  ...
  avatar.style.transform='translateX(-50%)'; avatar.style.width='auto';
  updateProgressBar(levelNum); loadPPEOptions(levelNum);
}
...
function showHint() {
  ...
  // Pick a random required PPE for this level
  const required = levelPPE[currentLevel] || [];
  if (required.length === 0) return;
  const randomId = required[Math.floor(Math.random() * required.length)];

  // Highlight the PPE in inventory
  const ppeImg = document.querySelector(`.ppe-item[data-ppe-id="${randomId}"]`);
  if (ppeImg) {
    ppeImg.classList.add("hinted");
  }
}

function loadPPEOptions(level){
  const inv=document.getElementById('ppe-inventory'); inv.innerHTML='';
  allPPE.forEach(item=>{
    const w=document.createElement('div'); w.classList.add('ppe-wrapper');
    const img=document.createElement('img'); img.src=item.src; img.alt=item.label;
    img.classList.add('ppe-item'); img.dataset.ppeId=item.id;
    img.addEventListener('click',()=>{img.classList.toggle('selected');sounds.selection.play();});
    const lbl=document.createElement('div'); lbl.textContent=item.label; lbl.classList.add('ppe-label');
    w.append(img,lbl); inv.appendChild(w);
  });
}

function startTimer(){
  clearInterval(timerInterval); countdownPlayed=false;
  const dsp=document.getElementById('timer');
  timerInterval=setInterval(()=>{
    if(totalTime<=0){ clearInterval(timerInterval); sounds.ticking.pause(); sounds.ticking.currentTime=0;
      if(document.getElementById('win-screen').classList.contains('hidden')) showTimeoutScreen(); return; }
    if(totalTime===10 && !countdownPlayed){ sounds.countdown.play(); countdownPlayed=true; }
    totalTime--; const m=Math.floor(totalTime/60); let s=totalTime%60; if(s<10)s='0'+s;
...
function submitPPE(){
  ...
  if(ok){ currentLevel++; totalTime-=30; sounds.score.play();
    currentLevel>7? showCertificate(): setupLevel(currentLevel);
  } else { sounds.wrongPPE.play(); showHazardScreen(); setTimeout(restartGame,2500); }
}
...
function showCertificate(){
  ...
  sounds.ticking.pause(); sounds.ticking.currentTime=0; sounds.gameWon.play();
}

function restartGame(){
  currentLevel=1; totalTime=300; countdownPlayed=false;
  clearInterval(timerInterval); sounds.ticking.pause(); sounds.ticking.currentTime=0; sounds.newGame.play();
